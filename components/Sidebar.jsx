...
const Sidebar = ({ expand, setExpand }) => {
  ...
  return (
    <div
      className={`flex flex-col justify-between bg-[#212327] bg-sidebar pt-7 transition-all z-50 max-md:absolute max-md:h-screen ${expand ? "p-4 w-64" : "md:w-20 w-0 max-md:overflow-hidden"}`}
    >
      <div>
        <div
        ...
        >
          ...
          >
            ...
            />
            <div
              className={` absolute w-max ${expand ? "left-1/2 -translate-x-1/2 top-12" : "-top-12 left-0"} opacity-0 group-hover:opacity-100 transition bg-black text-white text-sm px-3 py-2 rounded-lg shadow-lg poniter-events-none`}
            >
        ...
        >
          ...
          >
            <div className="relative w-max bg-black text-white text-sm p-3 rounded-lg shadow-lg">
              ...
              <div
                className={`w-3 h-3 absolute bg-black rotate-45 ${expand ? "right-2/4" : "left-4 -botton-1.5"}`}
              ></div>
            </div>
          </div>
          {expand && (
